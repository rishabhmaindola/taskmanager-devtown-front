...
const Page = (props) => {
  ...
  useEffect(() => {
    const fetchTasks = async () => {
      try {
        const response = await axios.get(`https://taskmanager-devtown-back.vercel.app/tasks?userId=${props.userId}`);
  ...
  const addTask = async () => {
    if (newTask.title && newTask.description && newTask.dueDate) {
      try {
        ...
        const response = await axios.post("https://taskmanager-devtown-back.vercel.app/task", {
          user:props.userId,
          title: newTask.title,
  ...
  const updateTask = async () => {
    if (
      ...
      try {
        await axios.put(
          `https://taskmanager-devtown-back.vercel.app/update/${editTask._id}`,
          editTask
        );
        setTasks((prevTasks) =>
          prevTasks.map((task) =>
  ...
  const deleteTask = async (taskId) => {
    try {
      ...
      await axios.delete(`https://taskmanager-devtown-back.vercel.app/delete/${taskId}`);
